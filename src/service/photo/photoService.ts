...
class PhotoService {
	...
	public async deletePhoto(photoId: number): Promise<void> {
		try {
			...
			CustomError.builder().setMessage(error.message).setDetailedMessage(error.message).setErrorType("Server Error").setStatusCode(404).build().throwError();
		}
		...
		try {
			...
			CustomError.builder().setMessage("Error deleting photo").setDetailedMessage(error.message).setErrorType("Server Error").setStatusCode(500).build().throwError();
		}
	}
